...
/*
Addon Name: Twitter Erweiterung
Description: Fügt einen Twitter-Beitragstyp hinzu und verarbeitet Tweets, um korrekte Links zu erhalten.
Author: WMS N@W
...
class A_twitter_addon extends Autoblog_Addon {
	...
	public function initialise_addon() {
		register_post_type( 'tweet', array(
			...
			'labels'              => array(
				...
				'add_new'            => __( 'Neuen Tweet hinzufügen', 'autoblogtext' ),
				'add_new_item'       => __( 'Neuen Tweet hinzufügen', 'autoblogtext' ),
				'edit'               => __( 'Bearbeiten', 'autoblogtext' ),
				'edit_item'          => __( 'Tweet bearbeiten', 'autoblogtext' ),
				'new_item'           => __( 'Neuer Tweet', 'autoblogtext' ),
				'view'               => __( 'Tweet anzeigen', 'autoblogtext' ),
				'view_item'          => __( 'Tweet anzeigen', 'autoblogtext' ),
				'search_items'       => __( 'Tweets suchen', 'autoblogtext' ),
				'not_found'          => __( 'Keine Tweets gefunden', 'autoblogtext' ),
				'not_found_in_trash' => __( 'Keine Tweets im Papierkorb gefunden', 'autoblogtext' ),
				'parent'             => __( 'Übergeordneter Tweet', 'autoblogtext' ),
			),
		) );
	}
	...
	public function add_footer_options( $key, $details ) {
		...
		$label = sprintf( '<p>%s</p><p style="height:43px">%s</p><p style="height: 30px">%s</p><p style="height:30px">%s</p><p>%s</p>',
			__( 'Aktiviere Twitter für diesen Feed', 'autoblogtext' ),
			'',
			__( 'Verbraucherschlüssel', 'autoblogtext' ),
			__( 'Verbrauchergeheimnis', 'autoblogtext' ),
			__( 'Benutzer Anzeigename', 'autoblogtext' ) );
		...
		$content = sprintf( '<p>%s</p><p>%s</p><p>%s</p><p>%s</p><p>%s</p>',
			...
			__( 'Um den Twitter Consumer Key & Secret Key zu erhalten, besuche bitte diese URL <a target="_blank" href="https://apps.twitter.com/">https://apps.twitter.com/</a>, neue App erstellen. <br/>Nachdem die App erstellt wurde, besuche bitte die Registerkarte API-Schlüssel. Dort findest Du die Informationen', 'autoblogtext' ),
				...
				jQuery(document).ready(function ($) {
					if ($('input[name="abtble[url]"]').size() > 0) {
						$('input[name="abtble[url]"]').attr({
							'placeholder': 'Wenn Du die Twitter-Erweiterung verwendest, wird die URL überschrieben und automatisch generiert',
							'disabled'   : 'disabled'
						});
					}
				})
...
class A_Widget_Recent_Tweets extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array( 'classname' => 'widget_recent_tweets', 'description' => __( 'Die neuesten Tweets auf Deiner Webseite', 'autoblogtext' ) );
		$this->WP_Widget( 'recent-tweets', __( 'Aktuelle Tweets', 'autoblogtext' ), $widget_ops );
	...
	function widget( $args, $instance ) {
		...
		$title = apply_filters( 'widget_title', empty( $instance['title'] ) ? __( 'Kürzliche Beiträge', 'autoblogtext' ) : $instance['title'], $instance, $this->id_base );
			...
			<ul class='tweets'>
				<?php while ( $r->have_posts() ) : $r->the_post(); ?>
					<li class='tweet'><?php the_content(); ?>
						<a href='<?php echo get_post_meta( get_the_id(), 'original_source', true ); ?>'>#</a> - <?php echo get_the_date() . __( ' um ', 'autoblogtext' ) . get_the_time(); ?>
					</li>
				<?php endwhile; ?>
			</ul>
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php _e( 'Anzahl der anzuzeigenden Tweets:', 'autoblogtext' ); ?></label>
